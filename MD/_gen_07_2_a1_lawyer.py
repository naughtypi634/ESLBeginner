"""Generate '07-2 定语从句练习 A1（北京律师版）' styled HTML + PDF.
...
Content source: MD/07-2-定语从句练习(A1-北京律师).md
...
SOURCE = MD_DIR / "07-2-定语从句练习(A1-北京律师).md"
HTML_OUT = MD_DIR / "_07-2-定语从句练习(A1-北京律师).html"
PDF_OUT = PDF_DIR / "07-2-定语从句练习(A1-北京律师).pdf"

TITLE = "定语从句练习 A1 · 北京律师版"
FOOTER_LABEL = "07-2"
...
def build_html(ladders) -> str:
    parts = [
        ...
        "<head>",
        '<meta charset="UTF-8">',
        f"<title>{TITLE}（07-2）</title>",
        "<style>",
        CSS,
        "</style>",
        "</head>",
        "<body>",
        f'  <div class="title-area"><div class="main-title">{esc(TITLE)}</div></div>',
    ]
...
FOOTER = (
    f'<div style="width:100%; font-size:8px; font-weight:700; '
    f'font-family:\'Helvetica Neue\',Helvetica,Arial,sans-serif; color:#000000; '
    f'text-align:right; padding-right:15mm;">'
    f'{FOOTER_LABEL} · <span class="pageNumber"></span>/<span class="totalPages"></span>'
    "</div>"
)
